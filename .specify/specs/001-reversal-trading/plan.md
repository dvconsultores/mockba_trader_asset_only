...
- **M3 sizing (Phase 2)** (amended per spec Q10, operator directive): capital
  = venue equity × `position_size_pct` (default 45%; 2 slots deploy 90%,
  ~10% loss buffer); spot qty = capital ÷ entry; Orderly notional = capital
  × `dex_leverage` (default 3x). Committed capital is 100% at risk; the
  structural stop bounds the loss, it does not shrink the position.
  Pre-order guards: venue min-notional (Binance NOTIONAL filter, Orderly
  per-symbol minimum) and a sanity spread check — order book is otherwise
  unused by design (structure method, not microstructure). Trade rejected if
  judge `rr < rr_min` or monthly trade count ≥ cap.
