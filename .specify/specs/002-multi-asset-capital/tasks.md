...
- [X] T008 Update `trade/pnl.py` — add `is_entry_blocked_per_pair(asset, venue, equity)` and `get_global_daily_pnl()` for global kill switch per research.md §5; keep backward-compatible `is_entry_blocked()` wrapper
...
- [X] T013 [US1] Update `bot.py` main loop — replace `get_asset_list()` + `auto_trade_binance`/`auto_trade_orderly` booleans with `get_active_pairs()` iteration; each pair uses its own `capital_<venue>` for `compute_slot_size()` per research.md §3
...
- [X] T016 [US2] Update `trade/pnl.py` — `get_daily_pnl()` and `get_consecutive_losses()` accept optional `asset` parameter for per-pair scoping per research.md §5; implement `check_global_daily_loss()` that sums PnL across all pairs and disables `trading_enabled` if global limit breached
- [X] T017 [US2] Verify and document per-pair state isolation in `trading_bot/spot_scalper.py` — confirm `_price_memory[asset]`, `_last_entry[f"{venue}:{asset}:{side}"]`, and `_peak`/`_trough` dicts are correctly keyed per (asset, venue). No code changes expected — existing design already per-asset
- [X] T018 [US2] Verify and document per-pair state isolation in `trading_bot/futures_scalper.py` — same verification as T017 for futures scalper module-level dicts. No code changes expected
- [X] T019 [US2] Update `bot.py` cycle-level logic — skip exchange equity queries for venues with zero active pairs (avoid wasted API calls); add `max_concurrent_positions` enforcement: count `load_all_positions()` across all pairs, block new entries if limit reached
...
- [X] T020 [US3] Add `/assets` command to `telegram.py` — inline keyboard showing asset list with DEX/CEX capital, active status (✅/❌), and open position count per contracts/api.md Telegram Bot Contract
- [X] T021 [US3] Add multi-step "Add Asset" flow to `telegram.py` — prompt for symbol → CEX capital → DEX capital → activate CEX? (Yes/No) → activate DEX? (Yes/No) → validate via `settings_rules` → save via `upsert_asset_config()` per contracts/api.md
- [X] T022 [US3] Add "Edit Asset" flow to `telegram.py` — select asset from inline list → show current values → inline buttons to edit each field (symbol not editable) → re-validate on each change
- [X] T023 [US3] Add "Remove Asset" flow to `telegram.py` — select asset → check open positions → block with message if positions exist → confirm deletion if flat → remove via `delete_asset_config()`
- [X] T024 [P] [US3] Rewrite `dashboard-ui/src/AssetManager.tsx` — replace comma-separated asset list with `AssetConfig[]` interface per data-model.md §TypeScript; each asset card shows symbol, DEX capital, CEX capital, active toggles per venue, open position count
- [X] T025 [P] [US3] Add edit form to `dashboard-ui/src/AssetManager.tsx` — inline editing of capital values and venue toggles; save button triggers `PUT /api/assets/{symbol}`; add form triggers `POST /api/assets`
- [X] T026 [US3] Implement `POST /api/assets/validate` in `dashboard/main.py` — dry-run validation without saving per contracts/api.md; used by Mini App for inline validation feedback
- [X] T027 [US3] Implement `POST /api/assets/{symbol}/force-save` in `dashboard/main.py` — skip balance check, log override prominently, return `force_saved: true` flag per contracts/api.md
...
- [X] T031 [US5] Implement allocation summary computation in `dashboard/main.py` `GET /api/assets` — query exchange equity for each venue, compute `total_allocated = SUM(capital WHERE active)`, `remaining = equity - total_allocated`, `active_pairs = COUNT(active)` per contracts/api.md; if balance query fails, set `remaining: null` and `balance_error` per venue
- [X] T032 [P] [US5] Add allocation summary display to `telegram.py` `/assets` list — show per-venue line: "CEX: $X allocated / $Y remaining (N assets)" per spec US5 acceptance scenarios
- [X] T033 [US5] Add `max_active_pairs` enforcement to `bot.py` — at cycle start, count active pairs from `get_active_pairs()`; if `len(pairs) > max_active_pairs`, log warning and iterate only up to the limit (order by symbol alphabetically for deterministic behavior). Add `max_concurrent_positions` enforcement (already in T019 — verify and cross-reference)
...
- [X] T034 Update `validate_all()` in `trade/settings_rules.py` to include per-asset capital checks — iterate `get_all_asset_configs()`, validate each row's capital/flags against the new rules from T006
- [X] T035 Update `validate_startup()` in `bot.py` — run T034's per-asset validation as part of the startup gate; if any asset has error-level violations, refuse to trade
- [X] T036 Remove legacy `get_asset_list()`, `add_asset()`, `remove_asset()` functions from `db/db_ops.py` — replaced by `asset_configs` CRUD; verify no remaining callers (check `telegram.py`, `bot.py`, `dashboard/main.py`)
- [X] T037 Remove legacy `assets` setting key references from `trade/settings_schema.py` and `trade/settings_rules.py` — ensure no code path reads the old comma-separated string
- [X] T038 Verify Constitution compliance — audit each of the 8 principles against the implemented changes per plan.md post-design re-check table; document any deviations
- [X] T039 Run quickstart.md scenarios 1–7 end-to-end — verify all pass in dry-run mode; fix any failures
- [X] T040 Line budget verification — count lines in `bot.py`, `regime.py`, `pnl.py`, `executor.py`, `spot_scalper.py`, `futures_scalper.py`; confirm total ≤ 1,500 (Constitution VII)
...
## Phase 9: Convergence — Remaining Work (2026-07-27)

**Purpose**: Gaps identified by `/speckit.converge` assessment against spec.md, plan.md, and current codebase. Ordered by severity (CRITICAL/HIGH first).

- [X] T041 [US3] Remove legacy `dex_slot_pct` and `cex_slot_pct` validation bounds from `dashboard/main.py` `_validate_setting()` function (lines 362, 389, 392) per FR-003 (contradicts)
- [X] T042 [US3] Remove `dex_slot_pct` and `cex_slot_pct` from `dashboard-ui/src/MiniSettings.tsx` `FREE_INPUTS` array (lines 155-156) and remove `capital` computation from legacy keys (line 321) per FR-003 (contradicts)
- [X] T043 [US3] Update `telegram.py` asset management — replace legacy `get_asset_list()`/`add_asset()`/`remove_asset()` with `asset_configs` CRUD (`get_all_asset_configs()`, `upsert_asset_config()`, `delete_asset_config()`). Add `/assets` command showing per-venue capital, active flags, and open position count per T020 (partial/contradicts US3)
- [X] T044 [US3] Add capital input fields to `dashboard-ui/src/AssetManager.tsx` add-asset form — currently hardcodes `capital_dex:0, capital_cex:0`; user needs to set capital when adding an asset per US1 AC1 (missing)
- [X] T045 [US3] Add inline capital editing to `dashboard-ui/src/AssetManager.tsx` asset cards — each card should allow editing capital_dex/capital_cex values and trigger `PUT /api/assets/{symbol}` per T025 (missing)
- [X] T046 [US3] Implement `POST /api/assets/validate` in `dashboard/main.py` — dry-run validation endpoint per contracts/api.md; returns validation result without saving (missing)
- [X] T047 [US3] Implement `POST /api/assets/{symbol}/force-save` in `dashboard/main.py` — skip balance check, log override, return `force_saved: true` per contracts/api.md (missing)
- [X] T048 [US4] Add migration summary Telegram notification to `db/db_ops.py` `migrate_legacy_assets()` — send notification via `trading_bot/send_bot_message.py` after successful migration per T030 (missing — already implemented in bot.py)
- [X] T049 [US5] Add per-venue allocation summary display to `telegram.py` `/assets` command — show "CEX: $X allocated / $Y remaining (N assets)" per spec US5 AC1-2 (missing)
- [X] T050 [US2] Verify per-pair state isolation in `trading_bot/spot_scalper.py` and `trading_bot/futures_scalper.py` — confirm dicts keyed correctly per (asset, venue); document findings per T017/T018 (missing)
